...
> **Local safety policy:** Treat every command below as an example. Never install packages, use `sudo`, load kernel modules, change kernel/driver/system settings, clear caches, download external code, or create profiling/build artifacts without the user's explicit approval. Prefer read-only diagnosis and repository-scoped outputs.
