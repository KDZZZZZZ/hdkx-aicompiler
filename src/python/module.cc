...
// 辅助函数：构造 Value
Value MakeValue(int64_t v) { Value val; val.v_int = v; return val; }
Value MakeValue(double v) { Value val; val.v_float = v; return val; }
Value MakeValue(const char* v) { Value val; val.v_str = v; return val; }
Value MakeValue(const Object* v) { Value val; val.v_object = v; return val; }
Value MakeNullValue() { Value val; val.v_object = nullptr; return val; }

// 将运行时对象恢复为最具体的 Python 句柄，确保 Device/NDArray 保留强类型接口。
py::object WrapObjectRef(const ObjectRef& object) {
...
// 模块初始化函数
void InitKXCRuntime(py::module_& m) {
