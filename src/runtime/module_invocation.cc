...
ModuleInvocationContract::ModuleInvocationContract(std::vector<ModuleInputContract> inputs, std::vector<ModuleTensorContract> outputs, std::vector<ModuleRuntimeExtentScalar> scalars, std::size_t budget, std::uint32_t version)
    ...
    std::size_t count=0;
    for(const auto& output:outputs_) { if(!ValidDType(output.dtype)||!output.device.defined()||!Pow2(output.alignment)||output.layout!="contiguous.row_major"||output.scope!="global"||output.logical.empty()||output.logical.size()!=output.physical.size()||output.logical.size()!=output.valid.size()) throw std::invalid_argument("module invocation output contract is invalid"); for(const auto* expressions:{&output.logical,&output.physical,&output.valid}) for(const auto& expression:*expressions) { expression.Validate(ranks); if((count += expression.unique_node_count()) > kMaxExpressions) throw std::invalid_argument("module invocation expression aggregate limit exceeded"); } }
    for(const auto& scalar:runtime_extent_scalars_) { if(!scalar.expression.defined()||!ValidDType(scalar.dtype)||!scalar.device.defined()||!Pow2(scalar.alignment)) throw std::invalid_argument("module runtime extent scalar is invalid"); scalar.expression.Validate(ranks); if((count += scalar.expression.unique_node_count()) > kMaxExpressions) throw std::invalid_argument("module invocation expression aggregate limit exceeded"); }
}
