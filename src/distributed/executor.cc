...
void ExecutionPlanExecutor::ExecuteComm(const ExecutionPlan& plan, const CommExecNode* comm) {
    ...
    if (comm->op_name == "device.copy") {
        ...
        if (comm->attrs.src_virtual_device.defined()) {
            src_worker = ResolveWorkerForVirtualDevice(
                plan, comm->attrs.src_virtual_device);
        }
        if (comm->attrs.dst_virtual_device.defined()) {
            dst_worker = ResolveWorkerForVirtualDevice(
                plan, comm->attrs.dst_virtual_device);
        }
        ccl_backend_->Copy(session_, src, dst, src_worker, dst_worker);
    } else if (comm->op_name == "device.allreduce") {
        ccl_backend_->AllReduce(session_, src, dst, comm->attrs.reduce_kind,
                                comm->attrs.in_group);
    } else if (comm->op_name == "device.broadcast_from_worker0") {
        ccl_backend_->BroadcastFromWorker0(session_, src, dst,
                                           comm->attrs.in_group);
    } else if (comm->op_name == "device.scatter_from_worker0") {
        ccl_backend_->ScatterFromWorker0(session_, src, dst,
                                         comm->attrs.in_group);
    } else if (comm->op_name == "device.gather_to_worker0") {
        ccl_backend_->GatherToWorker0(session_, src, dst,
                                      comm->attrs.in_group);
    } else if (comm->op_name == "device.send_to_worker") {
        ccl_backend_->SendToWorker(session_, src, dst,
                                   comm->attrs.root_worker);
    } else if (comm->op_name == "device.recv_from_worker") {
        ccl_backend_->RecvFromWorker(session_, src, dst,
                                     comm->attrs.root_worker);
    } else {
...
int ExecutionPlanExecutor::ResolveWorkerForVirtualDevice(const ExecutionPlan& plan,
    ...
    if (plan->placement.defined()) {
        int worker = FindWorkerForVirtualDevice(plan->placement, vd);
