/*! \file include/kxc/ffi/packed_func.h
...
public:
    ...
    explicit PackedFuncObj(const std::function<void(Args, RetValue*)>& f);
    KXC_OBJECT_DECLARE
};
...
template<> int64_t RetValue::As<int64_t>() const;
template<> double RetValue::As<double>() const;
template<> std::string RetValue::As<std::string>() const;
template<> ObjectRef RetValue::As<ObjectRef>() const;
template<> const char* RetValue::As<const char*>() const;

template <typename T>
typename std::enable_if<std::is_base_of<ObjectRef, T>::value, T>::type
