...
完整 8 层（650 kernel）暂时跑不完。本轮定位到两处**同一类缺陷**——按树遍历 Relay 的表达式 DAG，共享子表达式每被引用一次就重复展开一次，于是随共享点数量指数增长：

| 处 | 表现 | 状态 |
|---|---|---|
| IR 文本打印器 | `ToText` 在 MiniMind 上：1 层 2.3 MB，2 层 **784 MB**（337 倍）。而 `RunInstrumentedPass` 每个 pass 无条件渲染前后两次全图文本，3 层即 OOM（峰值 9.4 GB） | 已修（DAG 感知打印，共享节点只展开一次） |
| Relay mutator 重写 | `RelayPass::Mutate` 无共享记忆，`fold_constant` 自身指数级，且其输出会把 DAG 展开成树继续喂给下游每个阶段 | 已修（按节点指针记忆化） |

修复后**内存已线性**（约 117 MiB/层）。但**耗时仍超线性**：2→3 层节点数涨 1.5 倍、耗时涨约 50 倍。嫌疑在仍未记忆化、同样按树遍历 DAG 的 visitor / `InferTypePass` —— 它们不产出新节点所以不占内存，但重复遍历照样烧时间。这是当前唯一挡住完整 8 层的问题，正在跟进。

> 记账口径：3.3 的两处修复由并行会话 `hdkx-aicompiler-bb` 提交，不在本线的提交里；本文记录是因为它们直接决定第 1 项的可达规模。
...
5. **消除编译耗时的超线性**（当前唯一挡住完整 8 层的问题，见 §3.3）。方向是给仍按树遍历 DAG 的 visitor / `InferTypePass` 补共享记忆。做完才能跑完整 8 层并发布 E1 receipt。
