...
状态语义与计划一致：`unsupported < contracted < implemented < validated`。只有 `validated` 可作为本轨实际验证结论；`implemented` 表示代码和测试入口存在，但本机可能因环境门禁未执行。
...
| exact prefill | Relay composition 已实现：batched matmul + finite additive mask + stable softmax + batched matmul | CPU reference validated；LLVM/runtime 测试已实现但本机未运行 | unsupported | 只支持固定静态 profile，不支持 bucket/tail/valid extent |
| decode external K/V | token=1、外部静态 K/V composition 已实现 | CPU reference validated；LLVM/runtime 测试已实现但本机未运行 | unsupported | 不是 KV-cache append/update/page/lifetime 支持 |
| KV cache | 只有 append/page/capacity mock contract | reference trace validated | 未实现 | compiler/runtime 保持 gate；capacity 绝不冒充 valid context |
...
当前验证轨的最小垂直切片已完成；继续开放以下能力必须先解除对应硬门禁：
