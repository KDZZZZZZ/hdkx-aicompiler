...
> **W1 集成提交：** `e05d9d7`
>
> **状态：** W1 isolated/default-off baseline 已集成；W2 各 feature branch 实施中。Track05 已在 `feature/compiler-foundation-runtime-observability` 完成 generation-0 manifest/observability，尚待集成；其余 production adapters 和跨轨闭环未完成。本页不是 dynamic Shape、hot swap、dynamic control flow、Transformer 或 GPU attention 的生产能力声明。
...
4. **Runtime manifest/observability：** Track05 feature branch 已实现纯 Runtime per-call/task manifest、generation-0 exact ABI/entry/plan fingerprint 复验、结构化 fallback query/observer、task wait/launch/allocation/logical-retire/generation 事件和 Async manifest retention；Runtime 不 include Compiler，gate 仍默认 OFF，non-zero generation 仍拒绝。集成后仍须补 authoritative Track03 generation lease、CUDA numeric/pending retention 与 Compute Sanitizer，才能考虑默认开启。
