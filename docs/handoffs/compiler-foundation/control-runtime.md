...
>
> 状态：**default-OFF、CPU:0、static-exact、fixture-backed 的 experimental evidence**
>
> 边界：这不是 production control backend，也不生成或发布 production artifact。真实 Relay/TE/backend resolver 尚未接入；当前只用 typed `CompiledModule` launcher fixture 验证 resolved schema、adapter、executor 和生命周期。
...
ControlPlan v2 (unresolved, compiler preparation)
  -> api::BindControlPlanForRuntime(typed fixture module-entry bindings)
...
- `include/kxc/runtime/control_execution_plan.h`
  ...
  - `BoundControlKernel` 在 adapter 边界直接快照并强持有 ready fixture `CompiledModule`
    entry executable、该 entry 的 `KernelSignature`/launch metadata，并对 CPU constant payload
    按 ABI alignment 建立私有深拷贝；后续 launch 不再按 symbol 或 module constant table 回查；
  - 公开 `Constant()` 只返回独立深拷贝，调用方修改原 module constant 或返回对象都不会
    改变 executor 使用的私有 snapshot；zero-byte/empty payload 保持合法空 Storage；
  - resolved task 不含 `kernel_ref`、Relay op 名或可供 executor 解释的 symbol 字段。
- `include/kxc/compiler/control_flow.h`
  - `ControlKernelBinding` 提供 task id、typed fixture module entry、`binding_revision` 和按 ABI
    排列的 non-output value ids；`binding_revision` 只是 caller 提供的正整数诊断标签；
  - `BindControlPlanForRuntime` 首先完整验证 `ControlPlan v2`（包括 locator/provenance
    非空约束），然后按 task id 绑定；不解释 `kernel_ref`，也不将其用于 module-entry 选择/dispatch；
  - `binding_revision` 不来自 authority，不提供 lease、staleness/freshness 检查、in-flight
    version pin 或 hot-swap proof，不能称为 artifact generation。
...
本次没有把控制流塞进 ordered `ExecutablePlan`，也没有重解释 `FrozenTaskPlan v1` 的 DAG/version 字段。
...
- 全局 `kPureFreshKernelOutputsV1` effect model：source plan 保持 pure/read-only input 与
  constant contract，每次 kernel invocation fresh allocate outputs，executor 不做 storage reuse；
  它**不是物理 no-alias 证明**，不同 logical read-only graph inputs 可以共享同一 NDArray，
  Phi/loop forwarding 也会保留 selected storage，planner 不得把该枚举当 no-alias 依据；
...
- 每个 kernel 已绑定 ready fixture module entry、同一 signature/metadata object identity、
  `binding_revision > 0` 和精确 ABI role/value mapping；output ABI 顺序必须与 task outputs 完全一致且不可重复；
- constant 必须由 module constant role/key 精确绑定；bind 时验证 CPU device、Storage
  range/null-data/alignment，并立即复制到私有 snapshot；跨 entry 的同一 logical constant 按
  contract 和 bytes 一致性验证，不依赖源对象 identity。

显式拒绝：

- unresolved kernel、`kernel_ref` execution、缺失/重复/额外/non-kernel binding；
- `binding_revision == 0`、entry/signature/ordered-output/device 漂移；同 contract constant 的 ABI 重排也拒绝；
...
- completion context 强持有 frozen plan、`BoundControlKernel`、fixture module/launcher、
  binding revision label、私有常量 snapshot、输入、中间值、selected branch、Phi、loop
  carried/backedge、outputs 和所有先前 operation；这仍不是 authority lease；
...
- ON：仅启用本页限定的 experimental CPU static-exact fixture executor；
...
- focused control-flow suite：两种 gate 均 **6/6 passed**；runtime-plan suite 均 **4/4 passed**；
- include-layer check：passed；
- public-header self-compile：passed；
- `git diff --check`：passed。

本机两套 evidence build 均显式配置 `KXC_ENABLE_CUDA=OFF`、`KXC_ENABLE_LLVM=OFF`；因此没有
LLVM/CUDA codegen、numeric、hardware 或 sanitizer 证据。测试仅使用 synthetic typed
`CompiledModule` + `KernelSignature` + `KernelLauncher` fixture。experimental executor 不根据
`kernel_ref` 或 op 名 dispatch；测试会把 `kernel_ref` 改成无效 decoy 并证明 resolved
binding/输出不变。
...
- duplicate logical input operand、两个 logical read-only graph inputs 共享同一 NDArray、
  fresh kernel outputs/no internal reuse 与 ordered unique output ABI；
...
- bind 后修改原 module constant、修改公开 `Constant()` 返回副本，execution 仍读取 private
  snapshot；覆盖 zero-byte、CPU device 和显式 alignment；
- session/module/plan/input 局部 owner 销毁后 completion 仍保活 fixture launcher/storage，释放
  completion 后 launcher 正常析构，无引用环。

## 6. 尚未开放的生产门禁

本次只有 typed module fixture evidence，因此仍需后续独立的 compiler production adapter：
...
4. 将 Track 03 authority-issued generation lease 转移为 runtime opaque keepalive；当前
   `binding_revision` 仅为 fixture label，不检测 stale binding、不 pin authority version，也不证明
   hot swap/in-flight safety；接入真实 lease 前不得改称 generation；
...
在这些门禁完成前，能力声明严格限定为：**default-OFF、feature-gated、CPU static-exact、
fixture-backed resolved control executor 的 experimental evidence；不是 production backend，
不生成 production artifact，也不是通用 dynamic graph/runtime 或 production Relay control compilation。**
