...
>
> 状态：Track 04 的 ANF、capability gate、structured `If`/loop DTO、verifier、Relay `If` preparation lowering、deterministic reference executor 已完成；真实 task-DAG/runtime 接线仍被 Track 01/02/05 的跨轨契约阻塞。

## 1. 能力边界：三个“动态”不是同一件事

| 概念 | 本分支状态 | 精确定义 |
|---|---|---|
| 动态控制流 | **已完成 static-exact 语义层** | Relay 数据谓词 `If` 可降为经过验证的 `ControlPlan v1`；手写 frozen DTO 支持有界 loop、Phi 和 loop-carried values，并可由 reference executor 执行。当前 `RuntimeSession` 尚不执行这些控制任务。 |
...
### 2.2 `ControlPlan v1`：Track 04/05 frozen mock
...
ID、source locator、`kernel_ref` 只用于 plan routing、诊断和 mock artifact 定位，不是跨图 artifact semantic key。

### 2.3 Structured verifier

`ControlPlan::ValidateStaticExact()` / `VerifyControlPlan()` 检查：

- schema 必须是 v1；value/region/task id 唯一且非负；所有引用存在；
...
- v1 只接受 pure/fresh-output task：effect reads 必须与 inputs 相符，writes/allocates/host callback/device sync 被拒绝；`must_alias`/`may_alias` 被拒绝，`no_alias` pair 需合法且无重复。
...
InferType -> NormalizeToANF -> VerifyANF
          ...
          -> ControlPlan v1 lowering -> ValidateStaticExact
```
...
- duplicate graph output id 在 v1 明确拒绝。
...
测试 oracle 位于 `test/support/control_plan_reference_executor.h`，消费相同 frozen DTO，并注入一个最小 fake-kernel callback。它：
...
随后逐个执行 23 个已构建测试程序，全部通过，包括全部新增测试以及：
...
- `RuntimeSession` 不执行 `ControlPlan`。
- 没有真实 backend branch/loop task、event、copy、allocation 或 memory planning。
- 没有 Relay loop AST/frontend syntax；loop v1 当前由 frozen DTO fixtures 验证。
...
5. **设备谓词与多 stream。** CUDA/device predicate 需要 Track 05 显式 Copy/Event/Sync task；多 stream 需要 completion event 参与 liveness。v1 因此只接受 CPU predicate 和 `default` stream。
6. **artifact/module 绑定。** `kernel_ref` 目前是 deterministic mock locator，不是 artifact semantic key，也未绑定 `CompiledModule` generation。生产接线依赖 Track 01/03/05 的 semantic key、immutable generation 和保活协议。
...
2. **冻结 Track 05 adapter。** 由 Track 05 从 `ControlPlan::kSchemaVersion == 1` 读取 `ControlRegion`/`ControlTask`，映射到 runtime-only `FrozenPlanVariant`；adapter 不得 include Relay/TE/TIR，也不得按 kernel/op 名推断依赖。
3. **绑定 kernel artifacts。** 对每个 `kKernel` task 使用 unit semantic key + target/ABI/pipeline fingerprint 查找或编译 immutable artifact；`kernel_ref` 只保留 locator/provenance。把 selected symbol/generation 写入 frozen task。
4. **实现 control executor。** 首先只做单 device/single `default` stream：CPU predicate branch、condition-before-body loop、max-trip guard、Phi/loop-carried copy/bind。执行前必须同时通过 ControlPlan validator 和 Track 05 frozen-plan validator。
...
7. **只在门禁通过后增加 Compiler feature gate。** `Compiler::Compile` 默认仍拒绝 If；显式 control-flow compile mode 才调用 `LowerRelayToControlPlan -> Track05 adapter -> FrozenPlanVariant`。失败立即回到“明确拒绝”，不能把 If 放回 ordered ValueGraph plan。
...
9. **最后接动态 Shape。** 只有 Track 02/05 提供 ShapeProgram、exact/bucket applicability、physical capacity、valid extent、dynamic allocation 和差分测试后，才升级 schema 开放 dynamic Phi/output/shape-changing loop；v1 继续作为 exact fallback。
