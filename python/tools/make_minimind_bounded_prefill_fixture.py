#!/usr/bin/env python3
"""Import the actual eight-layer MiniMind prefill and generate bounded B/S references.
...
def main() -> None:
    ...
    if ([value.name for value in model.graph.input] != ["input_ids"] or
            [value.name for value in model.graph.output] != names or
            counts["Sigmoid"] != 8 or counts["Softmax"] != 8 or counts["MatMul"] != 73):
        raise ValueError("fixture requires the actual eight-layer MiniMind prefill export")
    ...
    shapes = [["B", "S", 6400]] + [["B", "S", 4, 96] for _ in names[1:]]
    dynamic = helper.make_model(helper.make_graph(
        list(model.graph.node), "minimind_actual_bounded_prefill",
        [helper.make_tensor_value_info("input_ids", onnx.TensorProto.INT64, ["B", "S"])],
    ...
    for value in list(representative.graph.input) + list(representative.graph.output):
        for dim in value.type.tensor_type.shape.dim:
            if dim.dim_param in {"B", "S"}:
                extent = {"B": 1, "S": 4}[dim.dim_param]
    ...
    onnx.save(dynamic, args.out / "prefill_dynamic.onnx")
    onnx.save(representative, args.out / "prefill_representative.onnx")
    save_imported_model(imported, args.out / "prefill.json", args.out / "prefill.params")
    (args.out / "export_receipt.txt").write_text(sha256(args.out / "prefill_dynamic.onnx") + "\n")
    evaluator = ReferenceEvaluator(dynamic)
    rng = np.random.default_rng(20260909)
    cases = [(1, 1), (1, 4), (2, 3), (3, 8)]
    files = ["prefill_dynamic.onnx", "prefill_representative.onnx", "prefill.json",
             "prefill.params", "export_receipt.txt", "cases.txt"]
    for case, (batch, sequence) in enumerate(cases):
        tokens = rng.integers(0, 6400, size=(batch, sequence), dtype=np.int64)
        filename = f"input_{case}.bin"
        (args.out / filename).write_bytes(tokens.tobytes())
        files.append(filename)
        for output, array in enumerate(evaluator.run(None, {"input_ids": tokens})):
            expected = (batch, sequence, 6400) if output == 0 else (batch, sequence, 4, 96)
            ...
            files.append(filename)
        print(f"ONNX reference B={batch}, S={sequence}: logits and 16 KV outputs", flush=True)
    ...
    receipt = {
        ...
        "stage": "full_bounded_prefill", "layers": 8, "input_names": ["input_ids"],
        ...
        "representative": [1, 4], "bounds": {"B": [1, 3], "S": [1, 8]}, "cases": cases,
        "seed": 20260909, "reference": "onnx.reference.ReferenceEvaluator(full dynamic prefill)",
        "onnx_version": onnx.__version__, "numpy_version": np.__version__,
        "sha256": {name: sha256(args.out / name) for name in files},
    }
    (args.out / "receipt.json").write_text(json.dumps(receipt, indent=2) + "\n")
    print(f"actual full prefill: {len(imported.function.nodes)} Relay source calls, "
