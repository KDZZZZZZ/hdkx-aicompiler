#!/usr/bin/env python3
"""Import the actual eight-layer MiniMind decode and generate bounded B/P references.

Preserve input-dependent shape controls. Only C++ restricted preparation may
prove and compile them; this tool performs ONNX import and reference execution.
"""
...
def main() -> None:
    ...
    shapes = [["B", 1, 6400]] + [["B", "T", 4, 96] for _ in names[1:]]
    dynamic = helper.make_model(helper.make_graph(
        list(model.graph.node), "minimind_actual_bounded_decode",
        [helper.make_tensor_value_info("input_ids", onnx.TensorProto.INT64, ["B", 1])] +
    ...
    for value in list(representative.graph.input) + list(representative.graph.output):
        for dim in value.type.tensor_type.shape.dim:
            if dim.dim_param in {"B", "P", "T"}:
                extent = {"B": 1, "P": 4, "T": 5}[dim.dim_param]
    ...
    onnx.save(dynamic, args.out / "decode_dynamic.onnx")
    onnx.save(representative, args.out / "decode_representative.onnx")
    save_imported_model(imported, args.out / "decode.json", args.out / "decode.params")
    (args.out / "export_receipt.txt").write_text(sha256(args.out / "decode_dynamic.onnx") + "\n")
    evaluator = ReferenceEvaluator(dynamic)
    rng = np.random.default_rng(20260909)
    cases = [(1, 0), (1, 1), (2, 4), (3, 8)]
    files = ["decode_dynamic.onnx", "decode_representative.onnx", "decode.json",
             "decode.params", "export_receipt.txt", "cases.txt"]
    for case, (batch, sequence) in enumerate(cases):
        tokens = rng.integers(0, 6400, size=(batch, 1), dtype=np.int64)
        ...
        for output, array in enumerate(evaluator.run(None, feed)):
            expected = (batch, 1, 6400) if output == 0 else (batch, sequence+1, 4, 96)
            ...
            (args.out / filename).write_bytes(array.tobytes())
            files.append(filename)
        print(f"ONNX reference B={batch}, P={sequence}: logits and 16 KV outputs", flush=True)
    ...
    seed_ids = rng.integers(0,6400,size=(1,4),dtype=np.int64)
    seed = ReferenceEvaluator(prefill).run(None,{"input_ids":seed_ids})
    if len(seed) != 17 or seed[0].shape != (1,4,6400):
        raise ValueError("prefill seed output contract mismatch")
    for output, array in enumerate(seed):
        if output != 0 and array.shape != (1,4,4,96):
            raise ValueError("prefill seed KV shape mismatch")
        array = array[:,-1:,:] if output == 0 else array
        if array.dtype != np.float32 or not np.isfinite(array).all():
            raise ValueError("invalid prefill seed payload")
        filename = f"loop_seed_{output}.bin"
        (args.out / filename).write_bytes(array.tobytes())
        files.append(filename)
    filename = "loop_seed_ids.bin"
    (args.out / filename).write_bytes(seed_ids.tobytes()); files.append(filename)
    previous = seed
    loop_steps = []
    for step in range(4):
        token = int(np.argmax(previous[0].reshape(-1,6400)[-1]))
        past = 4+step
        feed = {"input_ids":np.array([[token]],dtype=np.int64)}
        feed.update({value.name:previous[i] for i,value in enumerate(dynamic.graph.input) if i})
        previous = evaluator.run(None,feed)
        for output,array in enumerate(previous):
            expected = (1,1,6400) if output == 0 else (1,past+1,4,96)
            if array.shape != expected or array.dtype != np.float32 or not np.isfinite(array).all():
                raise ValueError("invalid greedy decode reference")
            filename = f"loop_ref_{step}_{output}.bin"
            (args.out / filename).write_bytes(array.tobytes()); files.append(filename)
        loop_steps.append((token,past))
        print(f"ONNX greedy reference step={step}, token={token}, P={past}",flush=True)
    (args.out / "loop_steps.txt").write_text("".join(f"{token} {past}\n" for token,past in loop_steps))
    files.append("loop_steps.txt")
    receipt = {
        ...
        "stage": "full_bounded_decode", "layers": 8, "input_names": [value.name for value in dynamic.graph.input],
        ...
        "representative": [1, 4], "bounds": {"B": [1, 3], "P": [0, 8]}, "cases": cases,
        "seed": 20260909, "reference": "onnx.reference.ReferenceEvaluator(full dynamic decode)",
        "prefill_source_sha256": sha256(prefill_path), "loop_seed_length": 4,
        "loop_steps": loop_steps, "loop_seed_owner": "ONNX prefill reference; LLVM consumes external KV inputs",
